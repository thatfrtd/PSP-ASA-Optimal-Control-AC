...
x_f = [zeros(2, 1); zeros(2, 1); pi / 2 + 4 * pi; 0];
...
glideslope_constraint = {round(N / 2) - 10:N, @(t, x, u, p) norm(x(1:2)) - x(2) / cos(glideslope_angle_max)};
hop_constraint_plus = {round(N / 2), @(t, x, u, p) x(5) - ([deg2rad(-90)] + 1e-5)};
hop_constraint_minus = {round(N / 2), @(t, x, u, p) [rad2deg(-90)] - 1e-5 - x(5)};
hop_constraint_angvel = {round(N / 2), @(t, x, u, p) x(6) + 0.1};
...
glideslope_STC_trigger_height = 0.01; % [km]
