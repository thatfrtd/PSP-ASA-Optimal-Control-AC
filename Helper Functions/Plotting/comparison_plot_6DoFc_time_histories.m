function [] = comparison_plot_6DoFc_time_histories(t, x, u, names, options)
arguments
    t
    x
    u
...
for c = 1:Nc
    plot(ax, t{c}(1:numel(u{c}(4, :))), u{c}(4, :) .* exp(x{c}(13, 1:numel(u{c}(4, :)))), LineWidth = 2, LineStyle = options.linestyle(c), DisplayName=names(c)); hold on
end
legend(ax, names)
