function [] = comparison_plot_6DoFq_ab_time_histories(t, x, u, names, options)
arguments
    t
    x
    u
...
ax = nexttile;
for c = 1:Nc
    gimbal_angle = rad2deg(vecnorm(u{c}(2:3, :)));
    ...
    plot(ax, t{c}(1:numel(gimbal_angle)), 1000 * u{c}(4, :), LineWidth = 2, LineStyle = options.linestyle(c), DisplayName="roll torque_" + "_{" + names(c) + "}"); 
end
legend(ax)
title("Gimbal Angle and Roll Torque vs Time")
xlabel("Time [s]")
ylabel("Gimbal Angle [deg] and Roll Torque [N m]")
grid on
